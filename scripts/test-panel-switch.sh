...
if check_active_panel "subscriptions"; then
    echo "  ✓ Subscriptions panel is active (status bar)"
else
    echo "  ✗ FAIL: Expected subscriptions panel to be active"
...
if check_frame_colors "subscriptions"; then
    ...
    echo "  ⚠ Warning: Could not verify frame color (may need visual check)"
fi
...
if check_active_panel "resourcegroups"; then
    echo "  ✓ Resource Groups panel is active (status bar)"
else
    echo "  ✗ FAIL: Expected Resource Groups panel"
    tail -2 /tmp/panel-check.txt
    FAILED=1
fi
if check_frame_colors "resourcegroups"; then
    echo "  ✓ Resource Groups frame is green"
fi
...
if check_active_panel "resources"; then
    echo "  ✓ Resources panel is active (status bar)"
else
    echo "  ✗ FAIL: Expected Resources panel"
    tail -2 /tmp/panel-check.txt
    FAILED=1
fi
if check_frame_colors "resources"; then
    echo "  ✓ Resources frame is green"
fi
...
if check_active_panel "main"; then
    echo "  ✓ Main panel is active (status bar)"
else
    echo "  ✗ FAIL: Expected Main panel"
    tail -2 /tmp/panel-check.txt
    FAILED=1
fi
if check_frame_colors "main"; then
    echo "  ✓ Main panel frame is green"
fi
...
sleep 0.3
if check_active_panel "subscriptions"; then
    echo "  ✓ Back to Subscriptions panel (status bar)"
else
    echo "  ✗ FAIL: Expected Subscriptions panel (cycle back)"
    tail -2 /tmp/panel-check.txt
    FAILED=1
fi
if check_frame_colors "subscriptions"; then
    echo "  ✓ Subscriptions frame is green (cycle complete)"
fi
...
sleep 0.3
if check_active_panel "main"; then
    echo "  ✓ Main panel is active (reverse)"
else
    echo "  ✗ FAIL: Expected Main panel (reverse)"
    tail -2 /tmp/panel-check.txt
    FAILED=1
fi
if check_frame_colors "main"; then
    echo "  ✓ Main panel frame is green (reverse)"
fi
...
sleep 0.3
if check_active_panel "resources"; then
    echo "  ✓ Resources panel is active (reverse)"
else
    echo "  ✗ FAIL: Expected Resources panel (reverse)"
    tail -2 /tmp/panel-check.txt
    FAILED=1
fi
if check_frame_colors "resources"; then
    echo "  ✓ Resources frame is green (reverse)"
fi
...
sleep 0.3
if check_active_panel "resourcegroups"; then
    echo "  ✓ Resource Groups panel is active (reverse)"
else
    echo "  ✗ FAIL: Expected Resource Groups panel (reverse)"
    tail -2 /tmp/panel-check.txt
    FAILED=1
fi
if check_frame_colors "resourcegroups"; then
    echo "  ✓ Resource Groups frame is green (reverse)"
fi
...
sleep 0.3
if check_active_panel "subscriptions"; then
    echo "  ✓ Back to Subscriptions panel (reverse)"
else
    echo "  ✗ FAIL: Expected Subscriptions panel (reverse cycle)"
    tail -2 /tmp/panel-check.txt
    FAILED=1
fi
if check_frame_colors "subscriptions"; then
    echo "  ✓ Subscriptions frame is green (reverse cycle complete)"
fi
...
if check_active_panel "subscriptions"; then
    echo "  ✓ Rapid Tab switching works correctly"
else
    echo "  ⚠ Warning: Rapid Tab switching may have issues (or panel was in different state)"
...
if check_active_panel "subscriptions"; then
    echo "  ✓ Final panel is subscriptions (status bar)"
else
    echo "  ✗ FAIL: Expected to end at subscriptions panel"
    FAILED=1
fi

if check_frame_colors "subscriptions"; then
    echo "  ✓ Subscriptions panel has green frame at end of test"
fi
...
if grep -q $'\033\[' /tmp/panel-colors-final.txt 2>/dev/null || \
   cat /tmp/panel-colors-final.txt | od -c | grep -q '\\033'; then
    echo "  ✓ ANSI escape sequences present (terminal colors working)"
fi
...
echo "==================================="
if [ $FAILED -eq 0 ]; then
    echo "✓ All panel switching tests passed!"
    exit 0
else
    echo "✗ Some panel switching tests failed"
    exit 1
fi
